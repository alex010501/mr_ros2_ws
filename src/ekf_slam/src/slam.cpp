...
Slam::Slam(const std::string &ns): Node(ns),
                                   map_frame(this->declare_parameter<std::string>("map_frame", "map")),
                                   feature_rad(this->declare_parameter<double>("feature_radius", 1.0)),
                                   v(0.0),
                                   ...
                                   P(Eigen::MatrixXd::Zero(X.size(), X.size()))
{
    ...
    odo_sub  = this->create_subscription<nav_msgs::msg::Odometry>("/odom", 1, std::bind(&Slam::on_odo, this, std::placeholders::_1));
    scan_sub = this->create_subscription<sensor_msgs::msg::LaserScan>("/scan", 1, std::bind(&Slam::on_scan, this, std::placeholders::_1));
    pose_pub = this->create_publisher<geometry_msgs::msg::PoseStamped>("slam_pose", 1);
...
void Slam::on_scan(const sensor_msgs::msg::LaserScan &scan)
{
    ...
    double delta_time = (scan_time - last_time).seconds();
    ...
    for (std::size_t i = 0; i < new_landmarks.size(); ++i)
    {
        ...
        else
        {
            ...
            {
                RCLCPP_ERROR_STREAM(this->get_logger(), "can not associate new landmark with any existing one");
            }
        }
    }
...
void Slam::publish_transform(const std_msgs::msg::Header &scan_header)
{
    ...
    // tf2::Transform используется для вычислений
    tf2::Transform tf_transform;

    // Расчет матрицы вращения и трансляции с использованием Eigen
    double angle = X(2);
    Eigen::Matrix2d R;
    ...
    // Устанавливаем значения в tf2::Transform
    ...
    // Создаем TransformStamped для публикации
    ...
    // Конвертация tf2::Transform в geometry_msgs::msg::Transform
